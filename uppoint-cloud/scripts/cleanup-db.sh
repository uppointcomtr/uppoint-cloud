...
INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS="${INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS:-}"
...
if [ -z "${INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS:-}" ]; then
  INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS="$(read_env_value "$ENV_FILE" "INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS")"
fi
...
INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS="${INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS:-90}"
...
if ! [[ "$INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS" =~ ^[0-9]+$ ]] || [ "$INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS" -lt 7 ]; then
  echo "[cleanup] HATA: INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS geçersiz (>=7 olmalı)." >&2
  exit 1
fi
...
# 9. AccountContactChangeChallenge — süresi dolmuş iletişim değişikliği challenge kayıtları
ACC_DELETED=$("${PSQL[@]}" -c "SELECT CASE WHEN to_regclass('\"AccountContactChangeChallenge\"') IS NULL THEN 0 ELSE (WITH d AS (DELETE FROM \"AccountContactChangeChallenge\" WHERE \"emailCodeExpiresAt\" < NOW() - INTERVAL '1 hour' RETURNING id) SELECT count(*) FROM d) END;")
echo "[cleanup] AccountContactChangeChallenge: ${ACC_DELETED} satır silindi"

# 10. RevokedSessionToken — süresi dolmuş JTI blacklist kayıtları
...
# 11. IdempotencyRecord — süresi geçmiş kayıtlar
...
# 12. NotificationOutbox — gönderilmiş/kalıcı hataya düşmüş eski kayıtlar
...
# 13. Legacy E2E kullanıcı kalıntıları — doğrulanmamış ve ilişkisiz test hesapları
...
# 14. InstanceProvisioningEvent — append-only provisioning event tablosu için retention temizliği
IPE_DELETED=$("${PSQL[@]}" -c "SELECT CASE WHEN to_regclass('\"InstanceProvisioningEvent\"') IS NULL THEN 0 ELSE (WITH d AS (DELETE FROM \"InstanceProvisioningEvent\" WHERE \"createdAt\" < NOW() - INTERVAL '${INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS} days' RETURNING id) SELECT count(*) FROM d) END;")
echo "[cleanup] InstanceProvisioningEvent (>${INSTANCE_PROVISIONING_EVENT_RETENTION_DAYS} gün): ${IPE_DELETED} satır silindi"
