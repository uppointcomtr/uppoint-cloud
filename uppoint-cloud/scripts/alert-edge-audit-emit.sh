...
if [ "${UPPOINT_CLOSED_SYSTEM_MODE:-true}" = "true" ] || [ -z "${UPPOINT_CLOSED_SYSTEM_MODE}" ]; then
  echo "[edge-audit-alert] closed-system mode active; external alert delivery skipped."
  exit 0
fi
...
VALUES (:'alert_id', 'EMAIL'::"NotificationChannel", :'recipient', :'subject', :'body', jsonb_build_object('scope','ops-edge-audit-emit-alert','severity','critical'), 'PENDING'::"NotificationOutboxStatus", NOW(), NOW());
SQL
