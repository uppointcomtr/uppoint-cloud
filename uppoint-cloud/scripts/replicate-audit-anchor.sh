...
UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION="${UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION:-}"
...
if [ -z "$UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION" ]; then
  UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION="$(read_env_value "$ENV_FILE" "UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION")"
fi
...
UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION="$(normalize_bool "${UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION:-false}")"

if [ "${UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION}" != "true" ]; then
  echo "[audit-anchor-replication] replication is disabled (set UPPOINT_ENABLE_AUDIT_ANCHOR_REPLICATION=true for explicit owner-approved runs)."
  exit 0
fi
