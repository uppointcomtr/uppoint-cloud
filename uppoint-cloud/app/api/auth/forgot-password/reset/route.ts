...
async function applyDeprecatedEndpointGuards(request?: Request): Promise<Response | null> {
  ...
  try {
    if (clientIp) {
      const ipResult = await checkRateLimit("deprecated-forgot-password-reset", clientIp, 60, 60);
      if (!ipResult.allowed) {
  ...
  try {
    ...
    return withRateLimitByIdentifier("deprecated-forgot-password-reset-agent", identifier, 120, 60);
  } catch {
    return null;
  }
}
  ...
  if (guardResponse) {
    ...
    if (guardResponse.status === 429) {
      await logAudit("rate_limit_exceeded", ip, undefined, {
        action: "deprecated-forgot-password-reset",
        scope: "ip_or_agent",
      });
    } else {
