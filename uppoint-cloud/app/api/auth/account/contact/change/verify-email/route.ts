...
function resolveVerifyEmailStatus(code: AccountProfileError["code"]): number {
  switch (code) {
...
export async function POST(request: Request) {
  return withIdempotency("auth:account-contact-change-verify-email", async () => {
    const ipGuard = await enforceFailClosedIpRateLimit({
      rateLimitAction: "account-contact-change-verify-email",
      rateLimitMax: 10,
      rateLimitWindowSeconds: 900,
      auditActionName: "account-contact-change-verify-email",
      auditScope: "ip",
    });
    ...
    if (challengeId) {
      const challengeRateLimit = await enforceFailClosedIdentifierRateLimit({
        rateLimitAction: "account-contact-change-verify-email-challenge",
    ...
    const userRateLimit = await enforceFailClosedIdentifierRateLimit({
      rateLimitAction: "account-contact-change-verify-email-user",
    ...
    try {
      ...
      await logAudit("account_contact_change_email_verified", ip, session.user.id, {
        result: "SUCCESS",
        step: "verify_email",
        type: result.type,
        challengeId,
      ...
      if (error instanceof AccountProfileError) {
        ...
        return NextResponse.json(
          ...
          { status: resolveVerifyEmailStatus(error.code) },
        );
      }

      await logAudit("account_contact_change_failed", ip, session.user.id, {
        reason: "ACCOUNT_CONTACT_CHANGE_VERIFY_EMAIL_FAILED",
        result: "FAILURE",
        step: "verify_email",
      });
